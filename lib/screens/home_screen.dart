...
class _HomeScreenState extends State<HomeScreen> {
    ...
    return Scaffold(
      ...
      body: BlocListener<PhotoBloc, PhotoState>(
        listener: (context, state) {
          if (state is PhotoSuccessState || state is PhotoErrorState) {
            _isFetching = false; // Reset fetching state after loading
          }
        },
  ...
  Future<void> _onRefresh() async {
    ...
    context
        .read<PhotoBloc>()
        .add(RefreshPhotos(photoPerPage: _photosPerPage));
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const Text("Do you want to download this image to your gallery?"),
          actions: <Widget>[
            TextButton(
              onPressed: () {
